...
Intuitively, this problem mimics the famous pebble game~\cite{Perumalla2013}, since one can not uncompute state $s_{n-1}$ after computing $s_n$.
...
Line 2 stores the result to the dirty ancilla, we get an approximately correct output.
Line 3``uncompute'' \texttt{out!} approximately with the contents in \texttt{anc!}, leaving a dirty zero state in register \texttt{out!}.
...
With this method, the implementation of $J_\nu$ is
...
Here, only a constant number of ancillas are used in this implementation, while the algorithm complexity does not increase.
...
This trick can be used extensively in many other application. It mitigated the artifitial irreversibility brought by the number system that we adopt at the cost of precision.
...
To obtain gradients, one can wrap the variable \texttt{y!} with \texttt{Loss} type and feed it into \texttt{ibessel\textquotesingle}
...
For example, the overhead in many iterative algorithms can often be removed with ``arithematic uncomputing'' trick without sacrificing reversibility as shown in the \texttt{ibessel} example in \Sec{sec:bessel}.
