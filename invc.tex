...
\begin{tabular}[t]{ll}
    ...
    $\sigma_P, s \Downarrow_p v$ & a NiLang statement $s$ under environment $\sigma_P$ is interpreted as value $v$ \\
    $\sigma_P, s \Downarrow_p^{-1} v$ & a NiLang statement $s$ under environment $\sigma_P$ is reverse-interpreted as value $v$ \\
...
Here, we do not define primitive instructions like $d_1 \mathrel{\odot}= x_f(d_2\cdots d_n)$, where $\odot \in \{+, -, *, /, \veebar\}$ here
because these instructions are evaluated as a regular julia function $ prime(\odot, f)(d_1, d_2 \cdots d_n) = (d_1 \odot f(d_2\cdots d_n), d_2 \cdots d_n)$ using the rule CALL.
As long as both the target function $ prime(\odot, f)$ and its inverse $\sim prime(\odot, f)$ are properly defined, it can be called in NiLang. On the other side, any non-primitive NiLang function will generate both the function self $x_f$ and its reverse $\sim x_f$, and this function can be used in NiLang.
The function arguments are data views. A data view is a modifiable memory, which can be a variable, a field of a data view, an array element of a data view, or a bijection of a data view. In NiLang, everything is mutable except for constants, where an error will be thrown if a statement tries to modify it.
NiLang does not allow the input data views of a function to map to the same memory, i.e. shared read and write is not allowed.
...
Even if a variables is used for shared reading, it can be dangerous in automatic differentiating. The share read of a variable induces a shared write of its gradient in the adjoint computing. For example, \texttt{y += x * x} will not give the correct gradient, but both \texttt{z ← zero(x); z += x; y += x * z; z -= x; z → zero(x)} and \texttt{y += x \textasciicircum 2} will.\end{document}
