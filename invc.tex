...
\end{theorem}
\begin{proof}
Consider a reversible function $\vy, b = f(\vx, a)$, where $a$ and $b$ are the input and output values of an ancilla. Since the ancilla is emptied for any input $\vx$, we have
\begin{align}
    \frac{\partial b}{\partial \vx} = \mathbf{0}.
\end{align}
...
Since the gradient fields are derived from the value fields of variables, discarding gradients should not have any effect to the \texttt{value} fields.
    The rest is to show $\texttt{grad(b)} \equiv \frac{\partial \vx^L}{\partial{b}}$ does not have effect in computing the gradients of function arguments. It can be seen from the back-propagation rule 
\begin{equation}
    \frac{\partial \vx^L}{\partial \vx} = \frac{\partial \vx^L}{\partial \vy}\frac{\partial \vy}{\partial \vx} + \frac{\partial \vx^L}{\partial b}\frac{\partial b}{\partial \vx},
\end{equation}
