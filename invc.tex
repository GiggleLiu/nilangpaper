...
\end{align}
where $\out$ represents the outputs of the program, $J^{\out}_{\vx^i} \equiv \frac{\partial \out}{\partial \vx^i}$ is the Jacobian to be propagated, and $J^{\vx^i}_{\vx^{i-1}}$ is the local Jacobian matrix. The Einstein's notation is used here so that the duplicated index $\vx^{i}$ is summed over.
