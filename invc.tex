...
This following is the NiLang implementation adapted from the above code.
...
Theoretically, the reversible routines do not nessesarily cost energy, the only energy bottleneck is copying gradient and outputs to the main memory.

We still face challenges in building energy efficient reversible hardwares. One is the reversible arithmetic instruction design, that is current floating-point adders and multipliers used in our computing devices are not exactly reversible.
...
Alternatives include fixed point numbers~\cite{FixedPointNumbers} and logarithmic numbers~\cite{Taylor1988,LogarithmicNumbers}, where logarithmic number system is reversible under \texttt{*=} and \texttt{/=} rather than \texttt{+=} and \texttt{-=}.
We also need instructions like \texttt{comefrom} as a partener of \texttt{goto} to specify the postconditions.
...
%Reversible hardware can be devices supporting reversible gates like the Toffoli gate and the Fredkin gate, or devices like an adiabatic CMOS with the ability to recover signal energy. The latter is known as the generalized reversible computing~\cite{Frank2005,Frank2017b}.
In the introduction, we introduced that the adiabatic CMOS can already be a better choice as the computing device in a spacecraft~\cite{Debenedictis2017}.
However, there are also some chanllenges in the hardware side too, one can find a proper summary of these challenges in \Ref{Frank2005b}.
%Since reversible programming is an exceptional platform for differential programming, building an energy-efficient artificial intelligence (AI) coprocessors would be also a promising direction to go.
...
An ancilla can sometimes carry a nonzero gradient when it is deallocated. As a result, the gradient program is not necessarily reversible.
...
Then, we need a number system to avoid rounding errors. Currently, we can simulate rigorous reversible arithmetics with the fixed-point number package \href{https://github.com/JuliaMath/FixedPointNumbers.jl}{FixedPointNumbers} \href{https://github.com/cjdoris/LogarithmicNumbers.jl}{LogarithmicNumbers} and \href{https://github.com/GiggleLiu/TropicalNumbers.jl}{TropicalNumbers}. A more efficient reversible design requires instruction-level design.
