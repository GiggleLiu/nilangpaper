...
The second stage generates the inverse function.
...
Consider a function $\vy = f(\vx)$, and an expected output $\vy^*$.
The goal is to find an input that $\vy^* = f(\vx^*)$. In traditional machine learning, we define a loss $\mathcal{L} = {\rm dist}(\vy^*, f(\vx^*))$ and minimize it.

Instead, we make use of the consistency relation directly by constructing a reversible program $\vy, \vg_y =  f_r(\vx, \vg_x)$, where $\vg_x$ and $\vg_y$ are ``garbage'' spaces that including parameters.
\begin{algorithm}[H]
    \KwResult{$\vg_x$}
    Initialize $\vx$ and garbage space $\vg_x$.\\
    ...
    \caption{Learn by consistency}
\end{algorithm}

Similar idea of training by consistency is used in self-consistent meanfield theory~\cite{} in physics.
The difficult part of self-consistent training is to find a consistency relation.
In reversible computing, one has a trivil self-consistency relation 
\begin{equation}
    \vg_x^* = \garbage(f_r^{-1}(\vy^*, \garbage(f_r(\vx^*, \vg^*_x)))),
\end{equation}
where $\garbage(\cdot)$ is a function for taking the garbage field.
Learn by consistency is not a silver bullet, it should be used with causion.
...
Loss function \texttt{f1} and \texttt{f2} computes $f(x, y) = e^{(y+x)} - e^x$ and stores the output in a new memory \texttt{out!}.
The only difference is $f_2$ ``uncompute'' $x$ arithmetically.
The task of training is to find a $y$ that make the output $x$ equal to the target value $1$.
...
The training fails because this function actually computes $f_1(out!, x, y!) = out! + x + e^{(y!+x)} - e^x, x, x+y!$, where the training parameter $x$ is completely determined by the output garbage space $x, x+y!$. Hence decreasing the initial value of $out!$ directly is a shorcut for training. While $f_2(out!, x, y!) = out! + x + e^{(y!+x)} - e^x, \left(x-log(exp(x))\right), x+y!$, parameters $x$ is not completely determined by the garbage space $\left(x-log(exp(x))\right), x+y!$.
...
It can be told by the mutual information between the input variable and output garbage space.
...
The gradient functions are implemented reversibally so that the gradients can be differentiated again to obtain Hessians.
In NiLang, one can use function \texttt{simple\_hessian} to obtain the Hessian matrix in this way.
