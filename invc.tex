...
\blue{add logarithmic number system solution to exp.}
%\begin{multicols}{2}
...
@i function ibesselj(out!, ν::Int, z::T; atol=1e-8) where T
    @zeros Int k fact_nu
    @zeros T anc1 anc2 anc3 anc4 anc5 halfz halfz_power_nu halfz_power_2 out_anc
...
Here, ancillas are created with the macro \texttt{@zeros}, where the first argument is the variable type and the rest are variable names. The above algorithm uses a constant number of ancillas, while the time overhead is also a constant factor.
...
Jie Pan for discussions about the issue of energy dissipation in CMOS technologies.
