...
\section{Introduction}\label{sec:intro}
    Computing the gradients of a numeric model $f : \mathbb{R}^m \rightarrow \mathbb{R}^n$ plays a crutial role in scientific computing. Automatic differentiation (AD) is the technique to obtain gradients automatically by propagating gradients with the chain rule. The propagation of gradients can happen in two directions. Consider a function $\vy = f(\vx)$, a tangent mode AD propagate the gradient forward and computes one column of its Jacobian $\frac{\partial \vy}{\partial x_i}$ efficiently, where $x_i$ is one of the input variables, whereas an adjoint mode AD propagates the gradients backward and computes one row of Jacobian $\frac{\partial y_i}{\partial \vec{x}}$ efficiently.~\cite{Hascoet2013}
...
%While the generated functions lower to reversible ``instructions'', these instructions are closed under the inverse operation ``$\sim$''. Hence all functions defined in NiLang are also closed under ``$\sim$'' operations.
