...
The preprocessor expands the symbol \texttt{$\sim$} in the postcondition field of \texttt{if} statement to the precondition, adds missing \texttt{@deanc} to ensure \texttt{@anc} and \texttt{@deanc} statements appear in pairs and  expands \texttt{@routine} macro.
Here, \texttt{@routine r <stmt>} records a statement to symbol \texttt{r}. In preprocessing stage, \texttt{$\sim$@routine r} will be replaced to \texttt{$\sim$<stmt>} for uncomputing. We will use macro extensively in the examples in \Sec{sec:example}.
...
The type that can be used in the reversible context is just a normal Julia type with extra requirements that its default constructor is not overwritten.
The constructor of a reversible type should be a reversible function.
Its inverse ``destructor'' unpacks data and deallocates derived fields.
One can use \texttt{@iconstruct} to define such a reversible constructor.
...
The first parameter \texttt{xx} is the actual input of constructor. The assign statement in argument list \texttt{gg = zero(xx)} initializes a new memory as a derived field. The function body is a reversible program that transforms \texttt{xx} and \texttt{gg} reversibly. Finally, the compiler appends a default constructor \texttt{DVar(xx, gg)} at the end to instantiate a new object as the return value.
The destructor that coverts an object of type \texttt{DVar\{T\}} to type \texttt{T} can be defined by reversing the above statements.%With the flexibility to use types, it is not necessary to use global stacks in our eDSL.
...
After computing the forward pass and backward pass, \texttt{~@routine getiloss} uncomputes the ancilla \texttt{iloss} and returns the location information to the loss variable.
...
With these infrastructures, a reversible program can be executed without suffering from rounding error. Meanwhile, they can reduce the time and space overheads significantly.
...
We construct a reversible program $\vy, \vp_y =  f_r(\vx, \vp_x)$, where $\vp_x$ and $\vp_y$ are ``garbage'' spaces which include trainable parameters and auxiliary parameters.
...
Learn by consistency can be used to handle discrete optimization. However, it is not a silver bullet, and should be used with caution.
