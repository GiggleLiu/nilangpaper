...
\end{table}
\namedseq{AssVar}{
    \sigma_{P \upharpoonright x}, e \Downarrow_e n_1 ~~~~ n = n_0 \llbracket \odot\rrbracket n_1 ~~~~ \odot \in \{+, -, *, /, \veebar\}
}{
    \sigma_P[x \mapsto n_0], x\mathrel{\odot}= e \Downarrow_p \sigma_P[x \mapsto n]
} 
...
\namedseq{WhileMain}{
    ...
    \sigma_P', (e_1, e_2, s) \Downarrow_{loop} \sigma_P''
}{
    \sigma_P, \wwhile (e_1, e_2)~~ s \eend\Downarrow_p \sigma_P'
}
...
\namedseq{Call}{        
    $
    \begin{aligned}
        F(x_f) = \textbf{function} \; x_f(x_1 \cdots x_n) \; \overline{s} \; \textbf{end}
        \\
        \varnothing[x_1 \mapsto  d_1 \cdots x_n \mapsto d_n] , 
        \textbf{begin} \; \overline{s} \; \textbf{end} \Downarrow_p \sigma_P'
    \end{aligned}
    $
}{
    \sigma_P, x_f(d_1 \cdots d_n) \Downarrow_p \sigma_P'
}

\namedseq{UnCall}{
    \sigma_P', P(id) \Downarrow_p \sigma_P
}{
    \sigma_P, ~s \Downarrow_p \sigma_P'
}

\namedseq{ComputeCopyUncompute}{
    \sigma_P, s_1 \Downarrow_p \sigma_P' ~~~~
    \sigma_P', \textbf{begin} ~~ s_2 \cdots s_n \eend  \Downarrow_p \sigma_P''~~~~
    \sigma_P''', s_1 \Downarrow_p \sigma_P''
}{
...
Since the \textbf{for} loop and \textbf{block} expression is same as the host language Julia, we do not elaborate here.

\namedseq{$+^\RR$}{
    \sigma_{P \upharpoonright x}, e \Downarrow_e n_1 ~~~~ n = n_0 \llbracket +\rrbracket n_1
}{
    \sigma_P[x \mapsto n_0], x\mathrel{-}= e \RDownarrow_p \sigma_P[x \mapsto n]
} 
...
\namedseq{$\veebar^\RR$}{
    \sigma_{P \upharpoonright x}, e \Downarrow_e n_1 ~~~~ n = n_0 \llbracket \veebar\rrbracket n_1
}{
    \sigma_P[x \mapsto n_0], x\mathrel{\veebar}= e \RDownarrow_p \sigma_P[x \mapsto n]
} 

\namedseq{IfTrue${}^\RR$}{
    \sigma_{P}, e_2 \Downarrow_e true ~~~~ \sigma_P, s_1 \RDownarrow_p \sigma_P' ~~~~ \sigma_P', e_1 \Downarrow_e true
}{
    \sigma_P, \iif (e_1, e_2)~~ s_1 \eelse s_2 \eend\RDownarrow_p \sigma_P'
}

\namedseq{IfFalse${}^\RR$}{
    \sigma_{P}, e_2 \Downarrow_e false ~~~~ \sigma_P, s_2 \RDownarrow_p \sigma_P' ~~~~ \sigma_P', e_1 \Downarrow_e false
}{
    \sigma_P, \iif (e_1, e_2)~~ s_1 \eelse s_2 \eend\RDownarrow_p \sigma_P'
}

\namedseq{WhileMain${}^{\RR}$}{
    \sigma_P, e_2 \Downarrow_e true ~~~~
    \sigma_P, e_1\Downarrow_e false ~~~~ 
    \sigma_P, s \RDownarrow_p \sigma_P' ~~~~
    \sigma_P', (e_1, e_2, s) \RDownarrow_{loop} \sigma_P''
}{
    \sigma_P, \wwhile (e_1, e_2)~~ s \eend\RDownarrow_p \sigma_P''
}
...
\namedseq{WhileBase${}^{\RR}$}{
    \sigma_P, e_1 \Downarrow_e true ~~~~
    \sigma_P, e_2 \Downarrow_e false
}{
    \sigma_P, (e_1, e_2, s) \Downarrow_p \sigma_P
}

\namedseq{Call${}^{\RR}$}{
     F(x_f) = \boldsymbol{function} \; x_f(x_1 \cdots x_n) \; \overline{s} \; \boldsymbol{end}
     ~~~~
     \varnothing[x_1 \mapsto  d_1 \cdots x_n \mapsto d_n] , 
     \boldsymbol{begin} \overline{s} \boldsymbol{end} \Downarrow_p^\RR \sigma_P'
}{
    \sigma_P, x_f(d_1 \cdots d_n) \Downarrow_p^\RR \sigma_P'
}
...
\namedseq{ComputeCopyUncompute${}^\RR$}{
    \sigma_P, s_1 \Downarrow_p \sigma_P' ~~~~
    \sigma_P', s_2 \RDownarrow_p \sigma_P''~~~~
    \sigma_P''', s_1 \Downarrow_p \sigma_P''
}{
    \sigma_P, \textbf{@routine} ~~
    s_1 ~~
    s_2 ~~
    \textbf{$\boldsymbol{\sim}$@routine}\RDownarrow_p \sigma_P''
}
