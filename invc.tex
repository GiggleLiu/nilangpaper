...
\section{Reverse computing style memory management}\label{timespace}
...
They also have many differences. When a polynomial overhead in time is allowed, reversible computing has a minimum space overhead of $O(S\log(T/S))$~\cite{Bennett1989,Levine1990,Perumalla2013}.
...
In practical using cases, we need to trade off space and time. The most successful checkpointing algorithm that widely used in automatic differentiation is the treeverse algorithm in \Fig{fig:tradeoff}(a). Where the computational process is binomially partitioned into sectors. At the begining of each sector, a snapshot is stored into the main memory. The states in the last sector is retrieved by above space efficient $O(T^2)$ algorithm. After that, the last checkpoint can be freed. The remaining sectors are further partitioned into $d$ sub-sectors. The earlier sectors has more quota of snapshots while the later has less, so that the total number of snapshots are remained the same.
Recursively apply this treeverse algorithm, the time and space overheads are both logarithmic. The approximated overhead in time and space are
\begin{align}
    T_c = tT\\
    S_c = dS
\end{align}
Where $T = \eta(t, d)$. One can easily find a solution that both $t$ and $d$ are logarithmic functions of time.

On the other side, the optimal tradeoff of reverse computing also has a recursive structure as shown in \Fig{fig:tradeoff} (b). The program is partitioned evenly into $k$ sectors, in each sector, the program is computed in a reversible compute-copy-uncompute style. Both compute and uncomputed are reversible programs that can be further devided in to $k$ sub-sectors.
\begin{align}
    T_r = T\left(\frac{T}{S}\right)^{\frac{\ln(2-(1/k))}{\ln k}}\\
    S_r = \frac{k-1}{\ln k}S\log\frac{T}{S}
\end{align}
Here, the overhead in time is polynomial. The treeverse like partition does not apply here because one can not complete the first sweep to create initial checkpoints without introducing any space overheads.
...
\ra{1.3}
    \scalebox{1.0}{
        \begin{tabularx}{\textwidth}{bsss}\toprule
            \thead{\textbf{Method}} & \thead{most time efficient \\ ($T'$/$S'$)} & \thead{most space efficient \\ ($T'$/$S'$)}\\
            \hline
            Checkpointing                           & $\bigO(T)/\bigO(T+S)$   & $\bigO(T^2)/\bigO(S)$   \\
            Reverse computing (irreversible limit)  & $\bigO(T)/\bigO(T+S)$   & $\bigO(T(\frac{T}{S})^{0.585})/\bigO(S\log(\frac{T}{S}))$ \\
            \bottomrule
        \end{tabularx}
    }
    \caption{$T$ and $S$ are the time and space of the original program. $T_c$ and $S_c$ are the time and space of the checkpointed program. $T_r$ and $S_r$ are the time and space of the reversible program.}\label{tbl:timespace}
\end{minipage}
\end{table}


\begin{minipage}{.88\columnwidth}
    \begin{lstlisting}[mathescape=true,caption={The Bennett's time-space trade-off scheme, the first argument is the collection of allocated states.
    Statement \texttt{$s_{i+1} \leftarrow 0$} allocates a zero state to the collection, while its inverse \texttt{$s_{i+1} \rightarrow 0$} discards a zero cleared state from the collection. Its NiLang implementation is in \App{app:bennett-nilang}.},label={lst:bennett}]
function bennett($\{s_1,...\}$, k, i, len)
    if len == 1
        $s_{i+1}$ ← 0
        $f_i$($s_{i+1}$, $s_i$)
    else
...
The reverse computing does not show advantage in worse-case complexity. However, it has several advantages in practise.
\textit{First}, It can making use of the reversibility to save memory. The above discussion assumes every operations is irreversible. However, most program contains a lot reversible instructions.
For example, the accumulation multilication operations in many BLAS functions and sparse matrix operations are reversible.
\textit{Second}, Reverse computing does not allocate automatically for users, user can optimize the memory access patterns on their own experience.
\textit{Third}, Reverse computing can operate on effective codes, so that it fits better with morden languages.
\textit{Fourth}, Reverse computing can utilize the existing compiler to optimize the code better.
\textit{Fifth}, Reverse computing brings user reversible thinking.
