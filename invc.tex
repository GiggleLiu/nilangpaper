...
Intuitively, this problem mimics the famous pebble game~\cite{Perumalla2013} considering the fact that product and division are considered as irreversible in NiLang, one can not deallocate $s_{n-1}$ after computing $s_n$.
However, here the case is different because $\mathrel{*}=$ and $\mathrel{/}=$ are arithmetically reversible to each other at nonzero points.
We can ``uncompute'' previous state $s_{n-1}$ by $s_{n-1} = \frac{n s_n}{x}$ approximately, and use the dirty ancilla in next iteration.
...
The two lines bellow the comment ``\texttt{\# arithmetic uncompute}'' uncompute variables \texttt{anc1} and \texttt{anc2} approximately.
The rounding errors are transfered to the output, while the reversibility is not affected since the inverse call at the last line of function uncomputes all ancillas rigorously.
This ``arithmetic uncomputing'' trick can be used extensively in many other application.
The reason why this trick works here is because from the mathematic perspective the state in $n$th step $\{\frac{x^n}{{\rm factorial}(n)}, x\}$ contains the same amount of information as the state in the $n-1$th step $\{\frac{x^{n-1}}{{\rm factorial}(n-1)}, x\}$, it is highly possible to find a route to compute the previous state from the current state.
This fact is not even related to the number system that we adopt.
...
In this paper, we introduce automatic differentiation on a reversible Turing machine (RTM) and a Julia eDSL implementation NiLang.
We show a program on an RTM can be differentiated to any order reliably and efficiently without sophisticated designs to memorize computational graph and intermediate states. We also deviced a ``arithmetic uncomputing'' trick to avoid the overhead of a reversible program in many practical cases.
