...
\textit{First}, It can making use of the reversibility to save memory. The above discussion assumes every operations is irreversible. However, most program contains a lot reversible instructions, e.g. the accumulation multiplication operations in many BLAS functions and sparse matrix operations. In \App{app:reversibility}, we show how to implement a unitary matrix multiplication without introducing any overheads in space and time.
...
\textit{Fifth}, Reverse computing encourages user reversible thinking, reversible thinking can lead user to a constant memory, constant time overhead implementation of chained multiplication algorithms as shown in \App{app:chainedmul}.
