...
Consider an irreversible program with computing time $T$ and peak run-time memory $S$. \Tbl{tbl:timespace} shows that both schemes suffer from a space overhead that linear to time when executing in time $\bigO(T)$.
...
\ra{1.3}
    ...
    \caption{$T$ and $S$ are the time and space of the original irreversible program. We put a ``worse case'' after the ``Reverse computing'' because the reversibility of the original program is not utilized.}\label{tbl:timespace}
\end{minipage}
\end{table}
...
But the following traits make it performs better in many practical applications.
...
For example, the accumulation multiplication operations in many BLAS functions and sparse matrix operations are reversible.
...
\textit{Third}, Reverse computing can operate on effective codes, so that it fits better with modern languages.
...
\textit{Fifth}, Reverse computing brings user reversible thinking, so that user can write better codes for reversing.
We exemplify these points in \App{app:rev-advantages}.
...
\subsection{Handling effective codes}
...
\subsection{Utilizing reversibility}
...
\subsection{Encourage reversible thinking}
