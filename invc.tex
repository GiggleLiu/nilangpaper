...
The following example shows how to define as reversible type.
...
Here, only the constructors marked with \texttt{@i} are reversible. The \texttt{@i} before \texttt{struct} keyword moves the definitions of \texttt{~B} functions to the outside of this type definition.

The type cast between two types can be defined convieniently with the macro \texttt{@icast}.

\begin{minipage}{.44\textwidth}
\begin{lstlisting}
julia> @pure_wrapper A

julia> @icast A(x) => B(x, g) begin
           g ← zero(x)
           g += identity(1)
       end
...
Here, we first define a simple reversible wrapper type \texttt{A} with macro \texttt{@pure\_wrapper}, then define the cast rule between \texttt{A} type and \texttt{B} type.
...
The inverse case that coverts an object of type \texttt{B} to type \texttt{A} is defined by reversing the above statements automatically.
%With the flexibility to use types, it is not necessary to use global stacks in our eDSL.

To access and manipuate the ``field''s of a data type, we introduce dataview. A dataview of variable can be the variable itself, a field of its view, an array element of its view, or a bijective mapping of its view.
...
\texttt{-arr[2].g}, \texttt{x} and \texttt{y} are all dataviews. In Julia language, \texttt{-grad(arr[2]) += x * y} statement will throw a syntax error because a function call \texttt{-(...)} can not be assigned, and \texttt{arr[3]} is an immutable type.
