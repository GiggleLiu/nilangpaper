...
\red{you may separate general principle with specific implementation choices. First state clearly what are the key requirement, and difficulties. Then introduce the design which address them. }
...
Second, we can built reversible BLAS and LAPACK, from matrix multiplication, tensor contraction to singular value decomposition (SVD) and eigenvalue decomposition (ED).
These operations are extensively used in physics~\cite{Xie2020,Liao2019}. However, for long time, many linear algebra AD rules suffer from the gradient exploding problem~\cite{Seeger2017,Wan2019,Hubig2019}.
Thrid, control problem in robotics~\cite{Giftthaler2017} also needs gradients. The control parameters are often floating point numbers rather than tensors.
Writting a control program reversiblly should boost the training performance a lot.
Fourth, NiLang is a strong candidate towards breaking the memory wall problem in machine learning.
Unlike the tradeoff schemes in traditional machine learning that use checkpointing and reversible primitives.
NiLang provides a more systematic and elegant time-space tradeoff scheme. It prefers inplace and memory efficient implementations
The implementation of unitary matrix costs zero extra memory,
Differentiating over reversible integrators~\cite{Hut1995, Laikov2018} without intermediate state caching is also possible.
In quantum simulator Yao~\cite{Luo2019}, all quantum gates are implemented as applying two level unitaries concequtively.
They can be implemented reversiblly with little effort, then we don't need a builtin AD engine anymore.
Besides, the time-space tradeoff can potentially be optimized at compiling stage as discussed above.

To solve the above problems better, NiLang can be improved from multiple perspectives. We call for better compiling that decreases granularity and hence reduces overhead, rigorous reversible floating point arithematics to let the reversibility free from rounding error.
...
It is also interesting to see how it can be combined with the high performance quantum simulator Yao~\cite{Luo2019}. It can provide control flow to Yao's quantum block intermediate representation, while Yao can provide the quantum features for NiLang.
Notice by introducing qantum rotation gates $R_y(\theta)$ and $R_z(\theta)$ in the reversible programming, we can make NiLang a path integral based universal quantum simulator. A wield fact is most quantum programming language assumes a classical coprocessor and use classical control flows~\cite{}.
We believe reversible control flow is also very important to a universal quantum computor.
...
\red{you need much better ending. Talk about broader perspective rather than technique details. }
