...
The memory is becoming a wall~\cite{Luo2019} that limits the scale of many applications on GPU. The computational power of a Nvidia V100 GPU can reaches 100 TFLOPS, which is the scale of a mini cluster. However, it memory is only 32GB. A better solution to memory menagement should be found.
...
\begin{figure}
    ...
    \caption{Data oriented computational graph that shows the compute-copy-uncompute design pattern. Circles connected by vertical lines are variables, while boxes and dots connected by parallel lines are operations. A dot on a line represents a control parameter while a box represents a mutable parameter.
    }\label{fig:ccu}
\end{figure}
