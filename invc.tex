will have a detailed discussion about the\documentclass[a4paper,superscriptaddress,11pt]{quantumarticle}
...
\end{enumerate}
    ...
    When it comes to needs about new primitives in scientific computing, they all rely on users to write backward rules for them.
    For example, in physics, the requirements to AD are diversed.
    \begin{enumerate}
        ...
        \item We need to back propagating SVD function in tensor network algorithms to study the phase transition problem~\cite{Liao2019}.
    ...
    \end{enumerate}
    None of the above packages can meet all these requirements by using predefined primitives only. Some progresses were made at the cost of lots of scientists effort. In the back propagation of dominant eigensolver~\cite{Xie2020}, authors manages to circumvent the sparse matrix issue by allowing users to provide the backward function for sparse matrix generator.
    The backward rules for SVD and eigensolvers has been manually derived recent years~\cite{Seeger2017,Wan2019,Hubig2019}. One can obtain the gradients correctly for both real numbers and complex numbers~\cite{Wan2019} in most cases, expect when the spectrum contains degeneracy.
    ...
    Tools like Tapenade~\cite{Hascoet2013} and Zygote~\cite{Innes2018, Innes2019} generate the adjoint code statically while putting variable on a stack called the wengert list. However, this approach has its own problem too.
    As the reverse mode AD requires storing intermediate result, the memory consumption of the program is typically O(n) where n is the runtime of the program.
    In many deep learning models like recurrent neural network~\cite{Lipton2015} and residual neural networks~\cite{He2016}, the depth can reach several thousand, where the memory is often the bottleneck of these programs.
    In traditional AD applications\blue{what is traditional AD applications}, a program that might do billions of computations will as well get a wengert list in the range of GBs, thus memory will become a bottleneck as well.
The memory management is becoming a wall~\cite{Luo2019} that limiting the scale of many applications on GPU. The computational power of a Nvidia V100 GPU can reaches 100 TFLOPS, competing a small cluster. However, its memory is only 32GB. A better solution to memory management should be found.
	for source to source ad, a direct consequene of caching intermediate states is the poor performance.
    an efficient source to source adjoint mode ad framework will give hope to solving many important issues.
    back propagating sparse matrix operations,
although their backward rules have been drived in recent years, these backward rules can not handle degenerate eigenvalues properly. Hopefully, the automatically generated backward rules do not have such problems.
...
In history, there have been some prototypes of reversible languages like Janus~\cite{Lutz1986}, R (not the popular one)~\cite{Frank1997}, Erlang~\cite{Lanese2018} and object-oriented ROOPL~\cite{Haulund2017}. % These languages have reversible control flow that allows user to input an additional postcondition in control flows to help programs run backward.
    ...
    After decades of efforts, reversible computing devices are very close to providing productivity now. For exmaple, adiabatic CMOS can be a better choice already in a spacecraft~\cite{Debenedictis2017}, where energy is more valuable than device itself.
    Reversible programming are also interesting to software engineers, it is a powerful tool to schedule asynchronious events~\cite{Jefferson1985} and debug a program bidirectionally~\cite{Boothe2000}.
    However, the field of reversible computing faces the difficulty of not enough funding recent decade~\cite{Frank2017}. As a result, little people know what this field did in machine learning field. People haven't connected it with automatic differentiation seriously even though they are so related. We aims to breaks the information barrier between the machine learning community and the reversible programming community in our work, and provides yet another strong motivation to develop reversible programming.
    ...
    In \Sec{sec:example}, we show several examples including Fibonacci number, Bessel function, unitary matrix multiplication and QR decomposition~\cite{Golub2012}. We show how to generate first order and second order backward rules for these functions.
...
\subsubsection{Memory management}
    ...
    The reversible stack is similar to a traditional stack except it zero-clears the variable after pushing and presupposes the variable being zero-cleared before popping.
\begin{figure}
...
In order to show how reversible memory manage works better, we introduce the memory oriented computational graph as shown in \Fig{fig:ccu}.
Notations are highly inspired by quantum circuit representations. A vertical line is a variable, it can be used by multiple operations, which means it is a hypergraph rather than a simple graph like DAG. When a variable is used by a function, we differentiate parameters by whether its value is changed by this function. We put a box if it is changed, other wise a dot.
Panel (a) shows that in the computing stage, the program in dashed box \texttt{X} is executed on space $x_{1\colon3}$ to compute desired result. In the copying stage, the contents in $x_3$ is read out to a pre-emptied memory $x_4$ through addition operation $\oplus$, because this is the piece of information that we care.
...
The interesting fact is, both $X$ and $\sim X$ are executed twice, which is not necessary. We can of cause cancel a pair of $X$ and $\sim X$ (the gray boxes), by doing this, we have to keep the memory $x_{1\colon3}$ during computing $f(x_4)$, i.e. additional space. The trade off between space and time will discuss the detail in \Sec{sec:timespace}.
...
For applications sensitive to rounding errors, we should take the effect of rounding error to reversibility seriously. Rounding errors can be suppressed by using other number systems, which will be discussed in \Sec{sec:hardware}
