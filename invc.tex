...
The following is a minimal example of compiling a reversible function to native Julia function with NiLang v0.3.1.
...
Here, macro \texttt{@i} generates three functions \texttt{f}, \texttt{$\sim$f} and \texttt{NiLangCore.isreversible}. \texttt{f} and \texttt{$\sim$f} are a pair of functions that are reversbile to each other. \texttt{$\sim$f} is an callable of type \texttt{Inv\{typeof(f)\}}, where the type parameter \texttt{typeof(f)} stands for th type of the function \texttt{f}.
...
The compilation of a reversible function to native Julia functions contain three stages: \textit{preprocessing}, \textit{reversing} and \textit{translation}.
...
\begin{figure}
    \centerline{\includegraphics[width=0.9\columnwidth,trim={0cm 0cm 0cm 0cm},clip]{compiling_v2.pdf}}
    \caption{Compiling process of NiLang, the body of complex number log as an example.}\label{fig:compiling}
\end{figure}

In the \textit{preprocessing} stage, the compiler pre-processes human inputs to reversible NiLang IR.
The preprocessor expands the symbol ``\texttt{$\sim$}'' in the postcondition field of \texttt{if} statement by copying the precondition, adds missing ancilla ``$\leftarrow$'' statements to ensure ``\texttt{$\leftarrow$}'' and ``\texttt{$\rightarrow$}'' appear in pairs inside a local scope, and handles the uncomputing macro \texttt{@routine}.
In the left most code box in \Fig{fig:compiling}, one uses \texttt{@routine <stmt>} statement to record a statement, and \texttt{$\sim$@routine} to insert the inverse statement for uncomputing.

In the \textit{reversing} stage, the compiler generates a reversed IR according to table \Tbl{tbl:revstatements}.
...
The condition expression in \texttt{if} or \texttt{while} statement is a tuple of precondition and postcondition, they are swapped during reversing.
...
\ra{1.3}
    ...
    \caption{The reversible statements in NiLang IR, where elements in the left column and those in the right column are reversible to each other.
...
In the \textit{translation} stage, the compiler translates this reversible IR as well as its inverse to native Julia code. It adds \texttt{@assignback} before each function call so that input arguments are effectively changed inplace, inserts codes for reversibility check, handle control flows.
At the end of a function definition, it attaches a return statement that returns all updated input arguments.
Now, the function is ready to execute on the host language.

\subsubsection{Types and Dataviews}
...
\begin{figure}
    \centerline{\includegraphics[width=0.6\columnwidth,trim={0.5cm 1cm 0 1cm},clip]{ad.pdf}}
    \caption{From left to right, the diagrams represent computing, uncomputing, Jacobian propagation, and Hessian propagation in the tensor network diagram.
    A big circle with three legs represents a Hessian, and a small circle with two legs represents a Jacobian.
    Dangling edges and connected edges stands for unpaired and paired labels respectively in Einstein's notation.}\label{fig:ad}
\end{figure}
...
To differentiate complex numbers, we need to re-implement all complex instructions reversiblly.
It can be seen easily from the following example that implements the .

\subsection{Second-order gradient}
Second-order gradients can be obtained by forward differentiating the adjoint program, adjoint differentiating the adjoint program, and Hessian propagation.
...
%\begin{figure}[htp]
%    \includegraphics[width=0.4\columnwidth,trim={0cm 1.5cm 0cm 0.5cm},clip]{fig5.pdf}
%\end{figure}

%\begin{figure*}
    %\centerline{\includegraphics[width=0.7\textwidth,trim={1cm 1cm 0 0},clip]{fig4.pdf}}
...
@i function sqdist(dist!, x1::AbstractVector{T}, x2::AbstractVector) where T
...
const L1 = [(1, 6), (2, 7), (3, 8), (4, 9),
    (5, 10), (1, 2), (2, 3), (3, 4), (4, 5),
    (1, 5), (6, 8), (8, 10), (7, 10), (7, 9),
    (6, 9)]

# disconnected bonds of a petersen graph
const L2 = [(1, 3), (1, 4), (1, 7), (1, 8),
    (1, 9), (1, 10), (2, 4), (2, 5), (2, 6),
    (2, 8), (2, 9), (2, 10), (3, 5), (3, 6),
    (3, 7), (3, 9), (3, 10), (4, 6), (4, 7),
    (4, 8), (4, 10), (5, 6), (5, 7), (5, 8),
    (5, 9), (6, 7), (6, 10), (7, 8), (8, 9),
    (9, 10)]
...
\ra{1.3}
    \scalebox{1.0}{
        \begin{tabularx}{\textwidth}{X X c c}\toprule
            ...
            %Manual & Adjoint & 83 & 0\\
            NiLang.AD & Adjoint & 231 & 0\\
            %NiLang.AD (GPU) & Adjoint & 1.4 & 0\\
            ReverseDiff & Adjoint & 7198 & 7.3\\
            Zygote & Adjoint & 22561 & 13.47\\
            Tapenade & - & 32 & 0\\
            Tapenade & Tangent & 30 & 0\\
            Tapenade & Adjoint & 111 & > 0\\
            \hline
            \bottomrule
        \end{tabularx}
    }
