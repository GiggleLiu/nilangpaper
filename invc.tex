...
To meet the diversed need, we need a general purposed AD (GP-AD) too to differentiate the whole programming language.
...
In machine learning, people also manage to not erasing informations that needed in the backward pass. 
In many neural networks such as recurrent neural networks~\cite{MacKay2018}, Hyperparameter learning~\cite{Maclaurin2015} and residual neural networks~\cite{Behrmann2018}, people also use reversibility to avoid caching intermediate state and reduce the memory cost.

However, we find when when people are trying to write a program in a free style, it is generally hard for an AD engine to avoid allocations.
Especially when the allocation is inside the loop, it will become a performance killer.
What's worse, allocating variables in a global stack will make the code impossible to be compiled to GPU even if it is generically written.
...
We will fist introduce the basic building blocks of a reversible programming language.
...
\begin{lstlisting}[mathescape=true,caption={A reversible adder},label={lst:complex}]
...
Macro \texttt{@i} generates two functions that reversible to each other \texttt{adder} and \texttt{$\sim$adder}, each defines a mapping $\mathbb{R}^2 \rightarrow \mathbb{R}^2$.
A reversible $\mathrel{+}=$ instruction is always defined as $y += f(args...)$, where $f$ is a mapping that allows to be irreversible, or just leave empty for identity mapping.
We can easily check these two functions are reversible to each other.
Then let's see a more advanced example of computing the complex valued log.
...
Here, \texttt{n ← zero(T)} is the ancilla allocation statement. One can input ``$\leftarrow$'' and ``$\rightarrow$'' by typing ``$\backslash$leftarrow[TAB KEY]'' and ``$\backslash$rightarrow[TAB KEY]'' respectively in a Julia editor or REPL.
\texttt{@routine} and \texttt{\sim @routine} are macros for computing-uncomputing. i.e. \texttt{~@routine} means running the statement after \texttt{@routine} backwards.
...
\begin{figure}
    \centerline{\includegraphics[width=0.7\columnwidth,trim={0 0cm 0 0cm},clip]{reversead.pdf}}
...
One does not need to define a similar function on \texttt{(:+=)(exp)} because macro \texttt{@i} will generate it automatically. Notice that taking inverse and computing gradients commute~\cite{Mcinerney2015}.
