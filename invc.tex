...
There has been some prototypes of reversible languages like Janus~\cite{Lutz1986}, R (not the popular one) and object oriented ROOPL, reversible instruction sets like Pendulum ISA and BobISA. Our eDSL borrows the design of reversible control flow in the Janus, meanwhile provides multiple dispatch based abstraction. With these additional features, differentiating over a general program requires less than 100 lines.
...
\section{Reversible computing stack}
...
In the lowest granuality, instruction design, we need ancilla bits, and uncomputing. Defining function like \texttt{iexp} requires uncomputing 4 ancillas. Deallocating the gradient further increase the granularity by $\sim 1$. After the training, the inverse training of whole programs should be done to deallocate all the memory used for training. The program complexity increase exponentially as the granuality increase.

These uncomputing can be flatten to a single uncomputing.
...
Logical reversible device and conditional reversibility
