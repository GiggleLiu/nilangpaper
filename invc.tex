...
The reversible \texttt{for} statement is similar to irreversible ones except that after executing the loop, and the program checks the values of these variables to make sure they are not changed. In the reverse pass, we exchange \texttt{start} and \texttt{stop} and inverse the sign of \texttt{step}.
\begin{figure}
...
This computational process could be diagrammatically represented as a DAG as shown in \Fig{fig:cgraphs}(\subref{fig:computational-graph}).
...
This reversible program can be diagrammatically represented as a memory oriented computational graph as shown in \Fig{fig:cgraphs} (\subref{fig:datagraph}).
...
\ra{1.3}
    ...
    The GPU time is estimated by broadcasting the gradient function on CUDA array of size $2^{17}$ and taking the average.}\label{tbl:besselj}
\end{minipage}
\end{table}
...
\begin{equation}
    J_{\nu}'(z) = \frac{J_{\nu-1} - J_{\nu+1}}{2}.
\end{equation}
Besides the efficient implementation, its high performance also lies in the fact that this functions is suited for tangent mode AD which has only one input.
...
In ReverseDiff and Zygote, the memory allocation on the heap is 7.3KB and 13.47KB, respectively. They are not all for checkpointing. Some of them are related to type instability, where Julia compiler fails to infer types.
...
The development of reversible compiling theory can benefit quantum compiling directly.
