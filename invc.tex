...
\namedseq{SET-VIEW-BIJECTOR}{
    z ~ fresh~~~~
    \sigma_P[z\mapsto v], (\sim e)(z) \Downarrow_e v'~~~~
    \sigma_P, v', d \Downarrow_{set} \sigma_P'
}{
...
because these instructions are evaluated as a regular julia function $ prime(\odot, f)(d_1, d_2 \cdots d_n) = (d_1 \odot f(d_2\cdots d_n), d_2 \cdots d_n)$ using the above CALL rule.
To reverse the call, the reverse julia function $\sim prime(\odot, f)$ should also be properly defined. On the other side, any non-primitive NiLang function defintion will generate two Julia functions $x_f$ and $\sim x_f$ so that this function can be used in NiLang.
Function arguments are data views, where a data view is a modifiable memory. It can be a variable, a field of a data view, an array element of a data view, or a bijection of a data view. In NiLang, everything is mutable except for constants, where an error will be thrown if a statement tries to modify it.
