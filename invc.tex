...
\begin{figure}
    ...
    \caption{Flow chart for reversible (a) \texttt{if} statement and (b) \texttt{while} statement. ``stmts'', ``stmts1'' and ``stmts2'' are statements, statements in true branch and statements in false branch respectively. ``pre'' and ``post'' are precondition and postconditions\violet{explain postcondition} respectively.}\label{fig:controlflow}
\end{figure}
...
the adjoint mode AD\violet{adjoint mode AD does not propagate Jacobians.} propagates the Jacobians in the reversed direction like
\begin{align}
    \begin{split}
...
Intuitively, this problem mimics the famous pebble game~\cite{Perumalla2013}\violet{explain pebble game.}, since one can not release state $s_{n-1}$ directly after computing $s_n$.
