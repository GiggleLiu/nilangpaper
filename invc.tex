...
\end{enumerate}
    ...
    \end{enumerate}
    None of the above packages can meet these requirements by using predefined primitives only. Some progresses were made at the cost of lots of scientists effort. In the back propagation of dominant eigensolver~\cite{Xie2020}, authors manages to circumvent the sparse matrix issue by allowing users to provide the backward function for sparse matrix generator.
    ...
    Souce code transformation based AD brings hope to free scientists from the labor of defining backard rules.
    However, this approach has its own problem too.
...
In history, there have been some prototypes of reversible languages like Janus~\cite{Lutz1986}, R (not the popular one)~\cite{Frank1997}, Erlang~\cite{Lanese2018} and object-oriented ROOPL~\cite{Haulund2017}. % These languages have reversible control flow that allows user to input an additional postcondition in control flows to help programs run backward.
    ...
    Reversbile programming are also intersting to software engineers, it is a powerful tool to schedule asynchronious events~\cite{Jefferson1985} and debug a program bidirectionally~\cite{Boothe2000}.
...
The interesting fact is, both $X$ and $\sim X$ are executed twice, which is not necessary. We can ofcause cancel a pair of $X$ and $\sim X$ (the gray boxes), by doing this, we have to keep the memory $x_{1\colon3}$ during computing $f(x_4)$, i.e. additional space. The trade off between space and time will discuss the detail in \Sec{sec:timespace}.
...
Source to souce automatic differentiation is very useful in differentiating sparse matrices. Sparse matrices can not easily benefit generic backward rules that defined for dense matrix, because generic rules does not keep the sparse structure.
...
This code is adapted from the irrversbile implementation in Julia package \texttt{SparseArrays}.
...
Here, we turn off the invertibility check off to achieve best performance.
One can always turn off this check after the debugging.
It is also possible to implement sparse matrix multiplication and other operations reversiblly.
