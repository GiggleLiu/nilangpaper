...
If we want to compute the adjoint of the complex valued \texttt{(:+=)(log)} function, one simply insert the gradient code into the reversed code in \Lst{lst:reversed-complex}.
As show in \Lst{lst:grad-complex}, the original code is highlighted with yellow background color, they now apply on the value field (\texttt{.x}) of the input value.
