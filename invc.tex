...
One may argue that easing through uncomputing is not nessesary here. This is not true for a general reversible program, because the intermediate states might be mutable and used in other parts of the program.
...
On the otherside, only checkpointing can achieve a zero space overhead by recomputing everything from the beginning $s_0$, with a time complexity $O(T^2)$.
...
NiLang is most useful in solving large scale scientic problems memory efficiently. In ~\citet{Liu2020}, people solve the ground state problem of a $28\times 28$ square lattice spinglass by re-writing the quantum simulator with NiLang.
There are some chanllenges in reverse computing AD too.
\begin{itemize}
    ...
    \item Although we show some examples of traning neural networks on GPU, the shared-reading of a variable is not allowed.
    \item NiLang's IR does not have variable analysis. The uncomputing pass is not always nessesary for the irreversible host language to deallocate memory.
