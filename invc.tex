...
\section{A benchmark of round-off error in leapfrog}
...
\begin{figure}
    ...
    \caption{Round-off errors in the final axes of planets as a function of the number of time steps. ``(regular)'' means an irreversible program, ``(reversible)'' means a reversible program (\Lst{lst:acc1}), and ``(accumulative)'' means a reversible program with the acceleration computed with accumulative errors (\Lst{lst:acc2}).}\label{fig:leapfrog}
\end{figure}
