...
\end{equation}
Here, the ``!'' after a variable is used to indicate that a variable is changed after the call of an instruction or function.
...
Even though \texttt{$\oplus({\rm identity})$}, \texttt{$\oplus(*)$}, \texttt{$\oplus(/)$} and their reverse together with control flows are sufficient to write an arbituary differentiable program.
