...
The following operational semantics for the forward and backward evaluation shows how a statement is evalued and reversed.
...
\namedseq{COMPUTE-COPY-UNCOMPUTE${}^\RR$}{
    %\sigma_P, s_1 \Downarrow_p \sigma_P' ~~~~
    %\sigma_P', \bbegin ~~ s_2 \cdots s_n \eend  \RDownarrowp \sigma_P''~~~~
    %\sigma_P''', s_1 \Downarrow_p \sigma_P''
    \sigma_P, \textbf{@routine} ~~
    s_1; ~~
    \sim\bbegin \overline s \eend ~~
...
We do not define primitive instructions like $d_1 \mathrel{\odot}= x_f(d_2\cdots d_n)$, where $\odot \in \{+, -, *, /, \veebar\}$ here
...
As long as both the target function $ prime(\odot, f)$ and its inverse $\sim prime(\odot, f)$ are properly defined, it can be called in NiLang. On the other side, any non-primitive NiLang function binded to symbol $x_f$ will generate both the function self $x_f$ and its reverse $\sim x_f$, and this function can be used in NiLang.
The function arguements are data views. A data view is a modifiable memory, which can be a variable, a field of a data view, an array element of a data view or a bijection of a data view. In NiLang, everything is mutable except for constants, where an error will be thrown if a statement tries to modify it.
...
% 
%%    \multicolumn{2}{c}{
%%\namedseq{WHILE-MAIN${}^\RR$}{
%%    \sigma_P, e_2 \Downarrow_e true ~~~~
%%    \sigma_P, e_1\Downarrow_e false ~~~~ 
%%    \sigma_P, \bbegin \overline s \eend \RDownarrowp \sigma_P' ~~~~
%%        \sigma_P', (e_1, e_2, \overline s) \Downarrow_{loop}^{-1} \sigma_P''
%%}{
%%    \sigma_P, \wwhile (e_1, e_2)~~ \overline s \eend\RDownarrowp \sigma_P'
%%}
%%}~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%%\\
%%    \multicolumn{2}{c}{
%%\namedseq{WHILE-REC${}^\RR$}{
%%    \sigma_P, e_1 \Downarrow_e true ~~~~
%%    \sigma_P, e_2 \Downarrow_e true ~~~~
%%    \sigma_P, s \Downarrow_p \sigma_P' ~~~~
%%        \sigma_P', (e_1, e_2, \overline s) \Downarrow_{loop}^{-1} \sigma_P''
%%}{
%%    \sigma_P, (e_1, e_2, \overline s) \Downarrow_{loop}^{-1} \sigma_P''
%%}
...
%%    \begin{aligned}
%%        F(x_f) = \ffunction \; x_f(x_1 \cdots x_n) \; \overline{s} \; \eend \\
%%        F(\sim x_f) = \ffunction \; (\sim x_f)(x_1 \cdots x_n) \;  \sim \bbegin \overline{s} \eend \; \eend \\
%%        \sigma_P, (\sim x_f)(d_1 \cdots d_n) \Downarrow_p \sigma_{P_n}
%%    \end{aligned}
%%    $
%%}{
%%    \sigma_P, x_f(d_1 \cdots d_n) \RDownarrowp \sigma_{P_n}
%%}
%%}
%\multicolumn{2}{c}{
%\namedseq{CALL${}^\RR$}{        
...
%        \\
%        \varnothing[x_1 \mapsto  v_1 \cdots x_n \mapsto v_n] , 
%        \bbegin \; \overline{s} \; \eend \RDownarrowp \sigma_{P_0}[x_1 \mapsto v_1' \cdots x_n \mapsto v_n']
%        \\
%        \sigma_{P_{i-1}}, v_i', d_i \Downarrow_{set} \sigma_{P_i}
%    \end{aligned}
%    $
