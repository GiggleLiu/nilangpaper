...
\begin{minipage}{.88\columnwidth}
\listingcaption{Insert the gradient code into \Lst{lst:reversed-complex}, codes highlighted with yellow background are for uncomputing.}\label{lst:grad-complex}
...
In practise, This transfromation is achieved not by source code transformation, but by changing the element type to \texttt{GVar} that containing two fields, value \texttt{x} and gradient \texttt{g}.
...
For example, the optimization passes can find out that for a irreversible device, uncomputing local variables \texttt{n} and \texttt{nsq} has no effect the to return values, so that it will skip the uncomputing pass automatically.
This is in contrast with the traditional approaches that focus a lot on data caching optimization.
NiLang's code is favorable to compilers because it does not rely on the global stack to reverse the code.
In this case, we also see the boundary between source code transformation and operator overloading can be vague.

One can define the adjoint of a primitive instruction as a reversible function either on the function itself or its inverse rather than defining them seperately, because the adjoint program of reversible functions are reversible to each other too.
\begin{align}
...
\begin{minipage}{.88\textwidth}
    \begin{lstlisting}[mathescape=true,caption={Adjoints for primitive instructions \texttt{(:+=)(sqrt)} and \texttt{(:-=)(sqrt)}.},label={lst:sqrt}, frame=tlrb]
