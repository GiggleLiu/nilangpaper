...
\ra{1.3}
    \scalebox{1.0}{
        \begin{tabularx}{\textwidth}{bsss}\toprule
            \thead{\textbf{Method}} & \thead{most time efficient \\ (Time/Space)} & \thead{most space efficient \\ (Time/Space)} & \thead{intermediate}\\
            \hline
            Checkpointing                           & $\bigO(T)/\bigO(T+S)$   & $\bigO(T^2)/\bigO(S)$                   & $\sim\frac{t^4}{24}/dS$\\
            Reverse computing (irreversible limit)  & $\bigO(T)/\bigO(T+S)$   & $\bigO(T(\frac{T}{S})^{0.585})/\bigO(S\log(\frac{T}{S}))$ & $T(\frac{T}{S})^{\epsilon(k)}/c(k)S\log(\frac{T}{S})$ \\
            Reverse computing (reversible limit)    & $\bigO(T)/\bigO(S)$     & $\bigO(T)/\bigO(S)$                     & $T/S$ \\
            \bottomrule
        \end{tabularx}
    }
...
The same amount of memory is required by the checkpointing method.
These two examples are used to demonstrate the two limits, in practise, we need to balance time and space.
